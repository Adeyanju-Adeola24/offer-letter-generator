import { formDataStore, showPreviewStore } from '../store'
...
export class App {
  ...
  private featureCard(icon: string, title: string, desc: string): string {
  ...
  private stepCard(num: string, title: string, desc: string): string {
    return `
  ...
  private initComponents() {
    ...
    if (headerRoot) this.header = new Header(headerRoot)
    if (formRoot) this.form = new Form(formRoot)
    if (previewRoot) this.preview = new Preview(previewRoot)
    if (footerRoot) this.footer = new Footer(footerRoot)
  }

  private loadSample() {
    const sample: Partial<FormData> = {
