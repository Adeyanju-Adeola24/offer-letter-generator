import { formDataStore, errorsStore, showPreviewStore, validateForm, resetForm } from '../store'
...
export class Form {
  ...
  private sectionFields: Record<string, string[]> = {
  ...
  private field(id: string, label: string, type: string, placeholder: string): string {
    return `
  ...
  private selectField(id: string, label: string, options: string[]): string {
    return `
      <div class="form-group">
        <label for="${id}" class="block text-sm font-medium text-gray-700 mb-1.5">${label}</label>
  ...
  private sectionHeader(icon: string, title: string, desc: string): string {
    return `
  ...
  private renderCandidateSection() {
    return `
  ...
  private renderCompensationSection() {
    return `
  ...
  private bind() {
    const form = this.el.querySelector('#offer-form') as HTMLFormElement
    const tabs = this.el.querySelectorAll('.section-tab')
    const nextBtn = this.el.querySelector('#next-btn') as HTMLButtonElement
    const prevBtn = this.el.querySelector('#prev-btn') as HTMLButtonElement
    const submitBtn = this.el.querySelector('#submit-btn') as HTMLButtonElement
    const resetBtn = this.el.querySelector('#reset-btn') as HTMLButtonElement
    const progressBar = this.el.querySelector('#progress-bar') as HTMLElement
    const sectionCurrent = this.el.querySelector('#section-current') as HTMLElement
    const sectionLabel = this.el.querySelector('#section-label') as HTMLElement
    ...
    const showSection = (index: number) => {
      ...
      tabs.forEach(t => {
        const i = parseInt((t as HTMLElement).dataset.index || '0')
        if (i === index) {
    ...
    tabs.forEach(tab => {
      tab.addEventListener('click', () => {
        showSection(parseInt((tab as HTMLElement).dataset.index || '0'))
      })
    })
    ...
    inputs.forEach(input => {
      input.addEventListener('input', () => {
        ...
        formDataStore.setState({ [el.name]: el.value } as Partial<FormData>)
    ...
    form.addEventListener('submit', (e) => {
      ...
      if (validateForm(data)) {
        ...
        this.showErrors()
        const firstError = this.el.querySelector('.border-red-500') as HTMLElement
        if (firstError) {
          const section = firstError.closest('.section-content') as HTMLElement
          if (section) {
    ...
    this.unsubs.push(
      errorsStore.subscribe(() => this.showErrors())
    )
  }
  ...
  private showErrors() {
    ...
    Object.entries(errors).forEach(([key, msg]) => {
      const input = this.el.querySelector(`[name="${key}"]`) as HTMLElement
  ...
  private syncForm() {
    ...
    this.el.querySelectorAll('.form-input').forEach(input => {
      const el = input as HTMLInputElement | HTMLSelectElement
      const key = el.name as keyof FormData
      if (key in data) {
